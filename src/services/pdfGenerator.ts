...
export const PdfGenerator = {
  ...
  async buildOfficialLetterDoc(
    ...
    const cityName = settings.city || 'Batumandi';
    doc.setFontSize(10);
    doc.setFont('helvetica', 'normal');
    doc.text(`${cityName}, ${dateStr}`, 130, y);
    y += 5;
    ...
    try {
      const origin = typeof window !== 'undefined' && window.location.origin
        ...
        : (settings.website || 'https://smkn1batumandi.sch.id');
      const verifyUrl = `${origin}/verify?code=${encodeURIComponent(qrText)}`;
