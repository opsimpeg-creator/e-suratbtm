...
export default function App() {
  ...
  const syncFromSpreadsheet = async (showToastFeedback = false, isBackground = false) => {
    setIsSyncingSpreadsheet(true);
    try {
      // 1. Fetch live submissions, complaints, users (excluding static school settings during background poll)
      await AppsScriptService.fetchDataFromAppsScript(isBackground);
  ...
  useEffect(() => {
    ...
    // Initial fetch from Google Spreadsheet on startup (fetch settings once)
    syncFromSpreadsheet(false, false);

    // Auto-poll Google Spreadsheet every 25 seconds for live submissions & complaints (background mode: preserves school settings)
    const syncInterval = setInterval(() => {
      syncFromSpreadsheet(false, true);
    }, 25000);
  ...
  // Dynamically synchronize browser tab title and favicon to current school branding
  useEffect(() => {
    const brand = settings.appName || 'LAPIS SMAKTUDI';
    const school = settings.schoolName || 'SMK NEGERI 1 BATUMANDI';
    document.title = `${brand} - ${school}`;
  ...
  }, [settings.appName, settings.schoolName, settings.logoUrl]);
