...
export const RequestManagement: React.FC<RequestManagementProps> = ({
  ...
  const filteredSubmissions = submissions.filter((req) => {
    ...
    if (statusFilter === 'all') {
      ...
      const dates = StorageService.extractActivityDates(req.formData);
      const isTemporal = (req.letterTypeName || '').toLowerCase().includes('dispensasi') ||
                         (req.letterTypeName || '').toLowerCase().includes('rekomendasi') ||
                         !!dates.activityEndDate;
      const res = StorageService.calculateArchiveStatus(dates.activityEndDate, isTemporal);
      matchesStatus = req.status === 'Selesai' && (res.isExpired || res.status.toLowerCase().includes('inaktif'));
    } else {
    ...
    if (typeFilter === 'all') {
      ...
      const selectedType = letterTypes.find((t) => t.id === typeFilter);
      const targetName = selectedType ? selectedType.name.trim().toLowerCase() : '';
      const reqName = (req.letterTypeName || '').trim().toLowerCase();
      
      matchesType =
        req.letterTypeId === typeFilter ||
        (!!targetName && (reqName === targetName || reqName.includes(targetName) || targetName.includes(reqName)));
    }
  ...
  const openDetailModal = (req: SubmissionRequest) => {
    ...
    if (siblings.length > 1) {
      ...
      setSyncToCollectiveSiblings(false);
    }
  };
  ...
  const handleSaveStatus = (e: React.FormEvent) => {
    ...
    const officialNumberToSave = modalStatus === 'Selesai' ? officialNumberInput : undefined;
    const officialDateToSave = modalStatus === 'Selesai' ? officialDateInput : undefined;

    // 1. Update the primary selected request
      ...
      collectiveSelectedIds.forEach((targetId) => {
        if (targetId !== selectedRequest.id) {
          StorageService.updateRequestStatus(
            ...
            modalStatus,
            actorName,
            modalNote || `Surat resmi diterbitkan secara kolektif (bersama ${selectedRequest.applicantName}).`,
            modalStatus === 'Ditolak' ? modalRejection : undefined,
            officialNumberToSave,
            officialDateToSave,
            uploadedOfficialFileUrl,
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-xs space-y-4">
        ...
        <div className="flex flex-wrap items-center gap-3 text-xs pt-2 border-t border-slate-100">
          ...
          {['all', 'Menunggu', 'Diproses', 'Selesai', 'Ditolak', 'Inaktif'].map((st) => (
            <button
              key={st}
              onClick={() => {
                setStatusFilter(st);
            ...
            >
              {st === 'all' ? 'Semua Status' : st === 'Inaktif' ? 'Inaktif / Selesai Kegiatan' : st}
            </button>
          ))}
          ...
          <table className="w-full text-left text-xs text-slate-700 relative">
            ...
            <tbody className="divide-y divide-slate-100 font-medium">
              {paginatedSubmissions.length === 0 ? (
                ...
                paginatedSubmissions.map((req, idx) => (
                  <tr key={req.id ? `${req.id}-${req.requestNumber || idx}` : `sub-row-${idx}`} className="hover:bg-slate-50 transition">
                      ...
                      <div className="flex flex-col gap-1 items-start">
                        ...
                        {req.status === 'Selesai' && (() => {
                          ...
                          const isTemporal = (req.letterTypeName || '').toLowerCase().includes('dispensasi') ||
                                             (req.letterTypeName || '').toLowerCase().includes('rekomendasi') ||
                                             !!dates.activityEndDate;
                          const archStatus = StorageService.calculateArchiveStatus(dates.activityEndDate, isTemporal);
                          return (
                            <ArchiveStatusBadge
                              status={archStatus}
                              endDate={dates.activityEndDate}
                            />
                          );
                        })()}
                      </div>
                    ...
                    <td className="p-4 font-mono text-slate-700">
                      {req.officialLetterNumber ? (
                        <div>
                          <div className="font-bold text-slate-900">{req.officialLetterNumber}</div>
                          {(() => {
                            const siblings = StorageService.getCollectiveSubmissions(req);
                            if (siblings.length > 1) {
                              return (
                                <span className="inline-flex items-center gap-1 mt-0.5 px-2 py-0.5 rounded-full text-[10px] font-bold bg-blue-50 text-blue-700 border border-blue-200">
                                  <Users className="w-3 h-3 text-blue-600" />
                                  Kolektif ({siblings.length} Siswa)
                                </span>
                              );
                            }
                            return null;
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  {modalStatus === 'Selesai' && (
                    <div>
                      ...
                      <input
                        ...
                        placeholder="Contoh: 16.616/SMKN1-BM/IX/2026"
                    ...
                    return (
                      <div className="sm:col-span-2 p-4 bg-white rounded-2xl border border-blue-200 shadow-xs space-y-3">
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                          <label className="flex items-center gap-2 font-extrabold text-blue-950 text-xs cursor-pointer">
                            <input
                              type="checkbox"
                        ...
                        {syncToCollectiveSiblings && (
                          <div className="space-y-2 pt-1 border-t border-slate-100">
                            <p className="text-[11px] text-slate-500">
                              Pilih siswa yang tergabung dalam nomor surat resmi <b>{officialNumberInput || '(belum diisi)'}</b> ini. Semua siswa terpilih akan otomatis memiliki nomor surat, file surat, dan QR Code verifikasi yang sama:
                            </p>
                            ...
                            <div className="max-h-48 overflow-y-auto divide-y divide-slate-100 border border-slate-200 rounded-xl bg-slate-50/50">
                              {potentialSiblings.map((sib) => {
                                const isSelected = collectiveSelectedIds.includes(sib.id);
                                const isPrimary = sib.id === selectedRequest.id;

                                return (
                                  <label
                                    key={sib.id}
                                    className={`flex items-center justify-between p-2.5 text-xs transition cursor-pointer ${
                                      isSelected ? 'bg-blue-50/80 font-bold text-blue-950' : 'hover:bg-slate-100 text-slate-700'
                                    }`}
                                  >
                                    <div className="flex items-center gap-2.5">
                                      <input
                                        ...
                                        onChange={(e) => {
                                          if (isPrimary) return;
                                          if (e.target.checked) {
                                            setCollectiveSelectedIds([...collectiveSelectedIds, sib.id]);
                                          } else {
                                            setCollectiveSelectedIds(collectiveSelectedIds.filter((id) => id !== sib.id));
                                          }
                                        }}
                                      ...
                                      <div>
                                        <div className="flex items-center gap-1.5">
                                          <span>{sib.applicantName}</span>
                                          {isPrimary && (
                                            <span className="text-[9px] bg-blue-700 text-white px-1.5 py-0.2 rounded uppercase font-bold">
                                              Pemohon Ini
                                            </span>
                                          )}
                                        </div>
                                        <div className="text-[10px] text-slate-400 font-mono font-normal">
                                          Resi: {sib.requestNumber} | Kelas:{' '}
                                          {sib.formData?.kelas || sib.formData?.progli || '-'}
                                        </div>
                                      </div>
                                    </div>
                  ...
                  <div className="sm:col-span-2 p-4 bg-white rounded-2xl border border-blue-200 shadow-xs space-y-3">
                    ...
                    <p className="text-[11px] text-slate-500 leading-relaxed">
                      Upload berkas surat resmi (format PDF, Scan/Foto JPG/PNG, atau DOC) yang dibuat manual oleh Staf TU agar dapat dibubuhi QR Code dan dipratinjau & diunduh oleh pemohon di halaman Cek Status.
                    </p>
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-3.5 bg-blue-50/80 border border-blue-200 rounded-xl">
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          <button
                            ...
                            className="px-3 py-1.5 bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold text-xs rounded-lg flex items-center gap-1.5 shadow-xs transition cursor-pointer"
                          ...
                          <button
                            ...
                            className="px-3 py-1.5 bg-blue-700 hover:bg-blue-800 text-white rounded-lg font-bold text-xs flex items-center gap-1.5 shadow-xs transition cursor-pointer"
                          >
                            <EyeIcon className="w-3.5 h-3.5" />
                            <span>Pratinjau</span>
                          </button>
              ...
              {previewModalFile.fileName.match(/\.(jpg|jpeg|png|gif|webp)$/i) || previewModalFile.fileUrl.startsWith('data:image/') ? (
                ...
                <div className="text-center p-8 bg-white rounded-2xl border border-slate-200 shadow-sm max-w-md">
                  ...
                  <button
                    type="button"
                    onClick={() => handleDownloadFile(previewModalFile.rawUrl || previewModalFile.fileUrl, previewModalFile.fileName)}
                    className="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold text-xs inline-flex items-center gap-2 shadow-sm cursor-pointer"
                  >
                    <FileDown className="w-4 h-4" />
