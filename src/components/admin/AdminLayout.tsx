...
export const AdminLayout: React.FC<AdminLayoutProps> = ({
  ...
  const getTabTitle = (tab: string) => {
    switch (tab) {
      ...
      case 'arsip':
        return 'Arsip Dokumen & Masa Berlaku';
      case 'jenis-surat':
          ...
          <div className="flex items-center justify-between border-b border-slate-800 pb-4">
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                <h2 
                  className="font-bold text-white text-xs sm:text-sm leading-snug line-clamp-2 break-words" 
                  title={settings.schoolName}
                >
                  {settings.schoolName}
                </h2>
                <p className="text-[11px] text-blue-400 font-medium mt-0.5">Portal Administrator</p>
              </div>
            </div>
      ...
      <main className="flex-1 overflow-y-auto">
        ...
        <header className="bg-white border-b border-slate-200 px-4 py-3 sm:px-6 sm:py-4 flex items-center justify-between shadow-2xs sticky top-0 z-30">
          ...
          <div className="flex items-center gap-2 sm:gap-4 text-xs font-medium shrink-0">
            {onRefreshSpreadsheet && (
              ...
              >
                ...
                <span className="hidden sm:inline">
                  {isSyncing ? 'Menyinkronkan...' : 'Segarkan Data'}
                </span>
              </button>
            )}
            ...
            <button
              type="button"
              onClick={() => setActiveTab('sync-spreadsheet')}
              className="hidden md:flex items-center gap-2 bg-slate-100 hover:bg-slate-200 px-3 py-1.5 rounded-xl text-slate-600 border border-slate-200 transition cursor-pointer"
              title="Klik untuk melihat atau mengganti target Spreadsheet ID & Apps Script"
            >
              <Database className="w-3.5 h-3.5 text-emerald-600" />
              <span>
                Spreadsheet ID:{' '}
                <b className="font-mono text-slate-800">
                  {settings.spreadsheetId ? `${settings.spreadsheetId.slice(0, 10)}...` : 'Belum diatur'}
                </b>
              </span>
            </button>
