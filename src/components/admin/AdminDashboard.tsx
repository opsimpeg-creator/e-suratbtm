...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({
  ...
  // Archive Documents & Expiry Statistics
  const archiveDocs = StorageService.getArchivedDocuments();
  const archiveAktifCount = archiveDocs.filter((d) => d.archiveStatus === 'Aktif').length;
  const archiveInaktifCount = archiveDocs.filter((d) => d.archiveStatus.toLowerCase().includes('inaktif')).length;
  const archivePermanenCount = archiveDocs.filter((d) => d.archiveStatus === 'Permanen').length;
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-gradient-to-r from-blue-900 via-blue-800 to-indigo-900 text-white p-6 sm:p-8 rounded-3xl shadow-xl flex flex-col md:flex-row items-center justify-between gap-6 relative overflow-hidden">
        <div className="space-y-2 z-10 text-center md:text-left">
          ...
          <h2 className="text-2xl sm:text-3xl font-extrabold tracking-tight">
            Dashboard Layanan {settings.appName || 'LAPIS SMAKTUDI'}
          </h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
        ...
        {/* Card 6: Arsip Surat (Di samping Surat Ditolak) */}
        <div
          onClick={() => setAdminTab('arsip')}
          className="bg-white p-5 rounded-2xl border border-slate-200 hover:border-purple-400 hover:shadow-md transition cursor-pointer space-y-3 group relative overflow-hidden"
          title="Klik untuk membuka Pusat Arsip & Masa Berlaku Dokumen"
        >
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-slate-500 uppercase group-hover:text-purple-700 transition">Arsip Surat</span>
            <div className="w-9 h-9 rounded-xl bg-purple-50 text-purple-700 flex items-center justify-center font-bold group-hover:bg-purple-700 group-hover:text-white transition">
              <Archive className="w-4 h-4" />
            </div>
          </div>
          <div className="flex items-baseline justify-between">
            <h3 className="text-2xl font-extrabold text-purple-800">{archiveDocs.length}</h3>
            <span className="text-[11px] text-purple-600 font-semibold flex items-center gap-1">
              <span className="text-emerald-600 font-bold">{archiveAktifCount}</span> Aktif
            </span>
          </div>
          <div className="text-[10px] text-slate-400 pt-0.5 border-t border-slate-100 flex items-center justify-between">
            <span>{archiveInaktifCount} Inaktif</span>
            <span>{archivePermanenCount} Permanen</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-8 bg-white p-6 rounded-3xl border border-slate-200 shadow-xs space-y-6">
          ...
          <div className="relative pt-6">
            ...
            {/* Bars container with explicit height */}
            <div className="h-44 flex items-end justify-between gap-2 px-1 relative z-10 border-b border-slate-200 pb-1">
              {monthlyData.map((d, i) => {
                const heightPercent = d.val > 0 ? Math.max(14, Math.round((d.val / maxVal) * 100)) : 3;
                return (
                  <div key={i} className="flex-1 flex flex-col items-center justify-end h-full group cursor-pointer">
                    {/* Badge value on top of the bar */}
                    <span 
                      className={`text-[10px] font-bold mb-1.5 transition-all transform group-hover:-translate-y-0.5 ${
                        d.val > 0 ? 'text-blue-700 bg-blue-50 px-1.5 py-0.5 rounded border border-blue-200 shadow-2xs' : 'text-slate-300'
                      }`}
                    >
                      {d.val}
                    </span>
                    ...
                    <div className="w-full flex-1 flex items-end justify-center px-0.5">
                      <div
                        ...
                        className={`w-full max-w-[36px] rounded-t-lg transition-all duration-300 ${
                          d.val > 0
                            ? 'bg-blue-600 hover:bg-blue-500 shadow-sm shadow-blue-500/20 ring-1 ring-blue-700/20'
                            : 'bg-slate-100 hover:bg-slate-200'
                        }`}
                        title={`${d.month}: ${d.val} permohonan surat`}
                      ></div>
                    </div>

                    {/* Month label */}
                    <span className={`text-[11px] font-semibold mt-2 ${d.val > 0 ? 'text-slate-800 font-bold' : 'text-slate-400'}`}>
                      {d.month}
                    </span>
                  </div>
                );
      ...
      <div className="bg-white p-6 sm:p-8 rounded-3xl border border-slate-200 shadow-xs space-y-6">
        ...
        <div className="divide-y divide-slate-100 overflow-x-auto">
          {recentSubmissions.length === 0 ? (
            ...
            recentSubmissions.map((req, idx) => (
              <div
                key={req.id ? `${req.id}-${req.requestNumber || idx}` : `recent-sub-${idx}`}
                onClick={() => {
