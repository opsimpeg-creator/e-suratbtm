    ...
    <div className="max-w-4xl mx-auto px-4 py-10 space-y-8">
      ...
      {foundRequest && (
        <div className="bg-white rounded-3xl border border-slate-200 shadow-xl overflow-hidden space-y-8 p-6 sm:p-8">
          ...
          {/* Collective Participants Section if multiple students share this official letter */}
          {(() => {
            const collectiveList = StorageService.getCollectiveSubmissions(foundRequest);
            if (collectiveList.length <= 1) return null;
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <h4 className="font-bold text-blue-950 text-xs uppercase tracking-wide">
                        Surat Kolektif ({collectiveList.length} Siswa Terdaftar)
                      </h4>
                      <p className="text-[11px] text-blue-800">
                        Nomor surat resmi ini diterbitkan bersama untuk beberapa rekan siswa berikut:
                      </p>
                    </div>
                  </div>
                  <span className="text-[10px] bg-white text-blue-700 font-bold px-2.5 py-1 rounded-full border border-blue-200 self-start sm:self-auto">
                    Nomor Surat: {foundRequest.officialLetterNumber}
                  </span>
                </div>

                <div className="overflow-x-auto rounded-xl border border-blue-200 bg-white shadow-xs">
                  <table className="w-full text-left text-xs">
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {collectiveList.map((st, idx) => {
                        const isCurrent = st.id === foundRequest.id;
                        const sClass =
                          st.formData?.kelas ||
                          st.formData?.progli ||
                          st.formData?.jurusan ||
                        ...
                        return (
                          <tr
                            key={st.id || idx}
                            className={isCurrent ? 'bg-blue-50/70 font-bold' : 'hover:bg-slate-50 transition-colors'}
                          >
                            ...
                            <td className="py-2.5 px-3">
                              <div className="flex items-center gap-1.5 text-slate-900">
                                ...
                                {isCurrent && (
                                  <span className="text-[9px] bg-blue-600 text-white px-1.5 py-0.2 rounded uppercase font-bold">
                                    Sedang Dilihat
                                  </span>
                                )}
                              </div>
                              <div className="text-[10px] text-slate-400 font-mono mt-0.5">
                                Resi: {st.requestNumber}
                              </div>
                            </td>
                            <td className="py-2.5 px-3 font-semibold text-slate-700">{sClass}</td>
                            ...
                            <td className="py-2.5 px-3 text-center">
                              {!isCurrent ? (
                                <button
                                  type="button"
                                  onClick={() => handleSearch(st.requestNumber)}
                                  className="text-[11px] bg-slate-100 hover:bg-blue-100 text-blue-700 hover:text-blue-900 font-semibold px-2 py-1 rounded transition cursor-pointer"
                                  title="Lihat detail permohonan siswa ini"
                                >
                                  Pilih Siswa
                                </button>
                              ) : (
